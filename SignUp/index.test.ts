...
it("should be succeed", async () => {
  ...
  const result = await run({} as Context, { body: credential } as ApiRequest<SignUpRequest>);
...
test("repeated registration", async () => {
  ...
  run({} as Context, { body: credential } as ApiRequest<SignUpRequest>);
  const result = await run({} as Context, { body: credential } as ApiRequest<SignUpRequest>);
